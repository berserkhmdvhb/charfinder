...
def test_output_format_fields() -> None:
...
def test_unicode_escape_format() -> None:
    results = list(find_chars("heart", verbose=False))
    for line in results:
        assert "\\u" in line
        assert line.startswith("U+")
...
def test_name_cache_exists() -> None:
...
def test_nfkd_decomposition_behavior() -> None:
    decomposed_query = normalize("\u00e9")
    assert decomposed_query == "E\u0301"

def test_fuzzy_threshold_effect() -> None:
...
def test_fuzzy_threshold_too_strict_returns_none() -> None:
...
def test_fuzzy_threshold_default_matches_explicit() -> None:
...
def test_non_string_query_raises_typeerror() -> None:
