...
def handle_find_chars(args: Namespace, query_str: str) -> tuple[int, dict[str, Any] | None]:
    ...
    try:
        ...
        if not query_str:
            return handle_empty_query(use_color)

        # Handle the search logic based on output format
        if args.format == "json":
            return handle_output_json(query_str, args, use_color, threshold)

        # Handle the main fuzzy search and output
        return handle_output_text(query_str, args, use_color, threshold)

    except KeyboardInterrupt:
        return handle_keyboard_interrupt(args.verbose, use_color)


def resolve_settings(args: Namespace) -> tuple:
    """Resolve color mode, debug flags, and threshold."""
...
def handle_empty_query(*, use_color: bool) -> tuple:
...
def handle_output_json(
    query_str: str, args: Namespace, *, use_color: bool, threshold: float
) -> tuple:
    """Handle JSON output directly via find_chars_raw."""
    rows = find_chars_raw(
        query=query_str,
        fuzzy=args.fuzzy,
        threshold=threshold,
...
def handle_output_text(
    ...
    """Handle regular search and output results."""
...
def handle_keyboard_interrupt(*, verbose: bool, use_color: bool) -> tuple:
